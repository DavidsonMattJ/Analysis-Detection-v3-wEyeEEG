...
job.plotGFX_groupFits_MSver4_raincloud =0; % MS version.Ncomms guidelines (raincloud)
...
testData= {GFX_TargPosData,GFX_TargPosData,GFX_RespPosData };
    ...
    cfg.pidx2= pidx2;
    cfg.plotlevel = 'GFX'; % plot separate figures per participant
    cfg.norm=0; %! Now including a norm parameter, to tidy plot.
    cfg.smoothON=0; % ! now smothing to help show individual data points.

    cfg.ylims = [-.15 .15]; % if norm =0;
    cfg.normtype= 'relative'; %relative, relchange, absolute, normchange
...
if job.plotGFX_groupFits_MSver4_raincloud ==1; % MS version. trial Data, targOns etc.
%%
% plot a pretty summary of the group level fourifer fit data.
% similar to the first plot job above, but including an example trial, \
% and trial onsets per gait bin. 
...
testData= {GFX_TargPosData,GFX_TargPosData,GFX_RespPosData };
    ...
    cfg.pidx2= pidx2;
    cfg.plotlevel = 'GFX'; % plot separate figures per participant
    cfg.norm=1; %! Now including a norm parameter, to tidy plot.
    cfg.smoothON=0; % ! now smothing to help show individual data points.
...
%     plot_FourierFitGFX_summary
    plot_GFX_groupFits_MSver4_raincloud(cfg,testData, GFX_FourierNull);
