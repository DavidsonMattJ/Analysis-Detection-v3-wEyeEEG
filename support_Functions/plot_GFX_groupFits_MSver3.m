function plot_GFX_groupFits_MSver3(cfg,testData, GFX_FourierNull)
...
params.scatSpacing= 4;% indices to skip before plotting ppant Data (avoids clutter).
params.boxlinewidth=.5;
params.plotScatter=1;
params.joinScatter=0; % join scatter points in timeseries?
...
ylim([min(gM)-3*sdrange max(gM)+3*sdrange])
% ylim([0 20]);
...
% subspots = [5,7;5,7;9,13]; % tile vertical 
% subspots=[5,6];
ic=1;
for igait=2%1:2
 ...
 % if normON
            ...
            if cfg.smoothON==1
                ...
                for isub= 1:size(dataIN,1)
                    ...
                    % weird edge artefacts, so first flip then smooth and
                    % extract.
                    tmp = [ppantData(isub,:), fliplr(ppantData(isub,:))];
                    sm_tmp= smooth(tmp,5);
                    
                    %take out original vector:
                    ppantData(isub,:)= sm_tmp(1:size(ppantData,2));
