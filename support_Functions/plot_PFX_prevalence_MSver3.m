...
function plot_PFX_prevalence_MSver3(cfg, testdata, PFX_FourierNull)
...
p=plot(cfg.Hzspace, ppantData(ippant,:),colp, 'LineWidth',2);
p.Color = [p.Color, .5];
ylim([0 .7]);
...
% add counts:
subplot(5,4,1 + (id-1));
text(4, .75, ['\bf\itn\rm\bf=' num2str(count2) ' \rm(NHST at ~2 cps)' ],'color', 'b');
set(gca,'xtick', round(cfg.Hzspace(1:5:length(cfg.Hzspace)),1), 'fontsize',12);
ylabel('R^2');
subplot(5,4,5 + (id-1));
text(4, .75,  ['\bf\itn\rm\bf=' num2str(count4) ' \rm(NHST at ~4 cps)' ], 'color', 'r');
set(gca,'xtick', round(cfg.Hzspace(1:5:length(cfg.Hzspace)),1), 'fontsize',12);
ylabel('R^2');
subplot(5,4,9 + (id-1));
set(gca,'xtick', round(cfg.Hzspace(1:5:length(cfg.Hzspace)),1), 'fontsize',12);
...
ylabel('R^2');
text(4, .75,  ['\bf\itn\rm\bf=' num2str(countOther) ' \rm(NHST > 5 cps)' ], 'color', 'm');
...
if id==1
subplot(5,4, [(13 +(id-1)), (17 +(id-1))]) ; hold on
