...
if strcmp(cfg.plotlevel, 'PFX')
    for ippant = 1:nsubs
        ...
        StimList= dataIN(ippant,1).gc_ContrVals_allwlking;       
...
if strcmp(cfg.plotlevel, 'GFX')
    ...
    fitsModel_WlkQuantile = zeros(nsubs,2, 3,200);
    ...
    for nGaits_toPlot=1%:2 % 2 not yet supported
        ...
        for ippant=1:nsubs
            ...
            searchGrid.alpha = StimLevelsFineGrain;
                ...
                fitsModel_WlkQuantile(ippant,iLR,iqnt,:) = ppantFit;
                end % iqnt
...
%             errP = CousineauSEM(GFX_headdata);
          ...
          StimLevelsFineGrain_Group=linspace(min(StimList_Group),max(StimList_Group),200);
            ...
            for iqnt= 1:size(NumPerQ,1)
               ...
               %plot data:
                lg(iLR)=plot(StimList_Group,ProportionCorrectObserved,['.'],'color',thisColr,'markersize',20);                
                %plot error:
                  errObs = CousineauSEM(squeeze(fitsObserved_WlkQuantile(:,iLR,iqnt,:)));                
                  errorbar(StimList_Group, ProportionCorrectObserved, errObs, 'k', 'linestyle', 'none');
%               
%add group fit:
                [paramsValues LL exitflag] = PAL_PFML_Fit(StimList_Group,round(ProportionCorrectObserved.*100)', ...
                ...
                groupFit = squeeze(nanmean(fitsModel_WlkQuantile(:,iLR,iqnt,:),1));
                end
