...
% pidx1=ceil(linspace(1,100,5)); % length n-1
% pidx2=ceil(linspace(1,100,11));% 

%now reducing size to tidy plots for NComms:
...
for ippant =1:nsubs
    ...
    load(pfols(ippant).name, 'subjID', 'trial_summaryTable',...
        ...
        'doubgait_ts_resamp', 'doubgait_ts_raw','gait_ts_raw');
    ...
    % mean head pos:
    GFX_headY(ippant).gc_raw = mean(gait_ts_raw,1, 'omitnan');
     %can also create a doubled version: 
    GFX_headY(ippant).doubgc_raw= mean(doubgait_ts_raw,1, 'omitnan');
    ...
    avWalkParams(ippant,1)= (mean(gait_ts_gData.gaitDuration(Ltrials_ts))+ mean(gait_ts_gData.gaitDuration(Rtrials_ts)))/2; %single step
    avWalkParams(ippant,2)= mean(gait_ts_gData.gaitDuration(Ltrials_ts)) + mean(gait_ts_gData.gaitDuration(Rtrials_ts)); %stride 
...
save('GFX_Data_inGaits', ...
    ...
    'subjIDs', 'pidx1', 'pidx2', 'gaittypes','GFX_EyeD','avWalkParams');%, '-append');
